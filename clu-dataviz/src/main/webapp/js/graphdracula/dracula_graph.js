	...
	node.hide = function() {
		...
		for (i in this.edges)
			(this.edges[i].source.id == id || this.edges[i].target == id)
					&& this.edges[i].hide && this.edges[i].hide();
	};
	node.show = function() {
		this.hidden = false;
...
Graph.Renderer.Raphael = function(element, graph, width, height) {
	...
	d.onmousemove = function(e) {
		...
		if (selfRef.isDrag) {
			...
			selfRef.isDrag.set.translate(clientX
					- Math.round(selfRef.isDrag.dx), clientY
					- Math.round(selfRef.isDrag.dy));
...
Graph.Renderer.Raphael.prototype = {
	...
	drawNode : function(node) {
		...
		if (node.shape) {
			...
			node.shape.translate(Math.round(point[0] - opoint.x), Math
					.round(point[1] - opoint.y));
			this.r.safari();
			return node;
		}/* else, draw new nodes */
		...
		if (!node.render) {
			node.render = function(r, node) {
				...
				shape = r.set().push(ellipse).push(r.text(0, 30, node.label));
				return shape;
			}
		}
		...
		shape.translate(Math.round(point[0] - (box.x + box.width / 2)), Math
				.round(point[1] - (box.y + box.height / 2)))
	...
	drawEdge : function(edge) {
		...
		if (edge.source.hidden || edge.target.hidden) {
			edge.connection && edge.connection.fg.hide() | edge.connection.bg
					&& edge.connection.bg.hide();
			return;
		}
...
Raphael.el.tooltip = function(tp) {
	...
	this.hover(function(event) {
		this.mousemove(function(event) {
			this.tp.translate(event.clientX - this.tp.o.x, event.clientY
					- this.tp.o.y);
			this.tp.o = {
				x : event.clientX,
				y : event.clientY
